...
## Проблемы
Ну, самая главная проблема была в сборке контейнера, это связано с тем, что докер почему-то зависал на сторче `self.transform(image)`, решил данную проблему с подбором параметров для гпу в контейнере.
